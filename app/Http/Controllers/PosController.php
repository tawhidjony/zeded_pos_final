...
{   use CommonDB, ResponseMessage;
    ...
    public function Create(){
        ...
        DB::raw("(SELECT SUM(purchases_products.quantity) FROM purchases_products 
                            GROUP BY purchases_products.product_id) as tqty"),
                DB::raw("(SELECT SUM(sale_details.qty) FROM sale_details
                            WHERE sale_details.product_id = purchases_products.product_id
                            GROUP BY sale_details.product_id) as sqty")
        ...
        if($request->cat_id != '*') {
            ...
            foreach ($all_pos_product as $singelProduct) {
                ...
                if(count($searchProduct) > 0){
                    ...
                    if(!empty($product->photo)){
                        $data[$product->id]['photo'] = asset('/images/'.$product->photo);
                    }else{
                        $data[$product->id]['photo'] = asset('/images/product-thumb.jpg');
                    }
                }
            ...
            foreach ($all_pos_product as $product) {
                ...
                if(!empty($product->product()->first()->photo)){
                    $data[$product->id]['photo'] = asset('/images/'.$product->product()->first()->photo);
                }else{
                    $data[$product->id]['photo'] = asset('/images/product-thumb.jpg');
                }
            }
    ...
    public function sale(Request $request ){
        ...
        $sellprodetails = array($sellProduct,$sell);
        $myJSON = json_encode($sellprodetails);
        if ($sell && $sellProduct){
            if( !isset($data['withoutprint']) ){
                $saleProducts= SaleDetails::all();
                $this->SendMsgViaMqtt('zededpos.de/pos/create',$myJSON);
            }
    ...
    public function getProductByNameKeyword(Request $request){
        ...
        if(!empty($srData)){
            $products_name = DB::select("SELECT purchases_products.*, products.name AS name, products.photo AS photo, categories.name AS cat_name, sub_categories.name AS subcat_name, child_tags.name AS child_tag_name FROM purchases_products 
    ...
    //MQTT Function
    private function SendMsgViaMqtt($topic, $message)
    {
        $mqtt = new Mqtt();
